...
uint8_t last_packs_ids[16384]; //last pack id for each unit in order to skip duplicating data
uint8_t packet_versions[16384]; //last pack id for each unit in order to skip duplicating data
float lost_packs_avg[16384];
...
int parse_ble_packet(uint8_t *pack, int maxlen, uECG_data *uecg_res)
{
	...
	if(!valid_list_inited) //init here so caller don't need to init manually - done only once
	{
		valid_list_inited = 1;
		valid_list_pos = 0;
		for(int x = 0; x < 16384; x++) valid_ids[x] = 0, lost_packs_avg[x] = 0;
	}
	...
	if(!is_good) //let's see if it has name uECG - depending on version, in can be in 2 places
	{
		...
		{//it's an uECG - add to list
			...
			packet_versions[valid_list_pos] = 0;
	...
	if(is_good)
	{
		...
		int packet_version = packet_versions[unit_index];
		if(pack[ppos+1] == 8 && packet_version == 0) //old protocol sends device name in every packet
			packet_version = 1;
		...
		if(pack[ppos+1] != 255 && packet_version == 2) return 0;
		
		packet_versions[unit_index] = packet_version;
...
//		printf("%d %d %d %d %d\n", data_pack[0], data_pack[1], data_pack[2], data_pack[3], data_pack[4]);
		...
		if(last_packs_ids[unit_index] == packet_id)
		{
			printf("duplicate\n");
			return -1;			
		}
		printf("pack id %d version %d dev idx %d\n", packet_id, packet_version, unit_index);
		...
		if(packet_version == 2)
			pack_data_count = 19;//23; //new version has 23 ECG data bytes
...
//		pack_data_count = 19;
		...
		int pp = 5;
		int v0 = data_pack[pp]*256 + data_pack[pp+1]; //first value
		if (v0 > 32767)
			v0 = -65536 + v0;
		pp += 2;
